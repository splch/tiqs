...
class OperatorFactory:
    ...
    def create(self, mode: int) -> qutip.Qobj:
        r"""Bosonic creation operator $a^\dagger$ for the
        given motional mode."""
        idx = self._mode_index(mode)
        dim = self.hs.fock_dim(mode)
    ...
    def number(self, mode: int) -> qutip.Qobj:
        r"""Number operator $\hat{n} = a^\dagger a$ for the
        given motional mode."""
        idx = self._mode_index(mode)
        dim = self.hs.fock_dim(mode)
