...
class GatePulse:
    ...
    For composite gates, the hamiltonian field contains a list of
    sequential (Hamiltonian, duration) pairs to be applied in order.
    """

    hamiltonian: Union[qutip.Qobj, list]
    duration: float
...
def ry_gate(
    ...
    """Rotation about Y by angle theta: R_y(theta) = exp(-i*theta*sigma_y/2)."""
...
def _rotation_hamiltonian(
    ...
    """Hamiltonian for a rotation about an axis in the x-y plane at angle `phase`.
...
def sk1_composite_gate(
    ...
    """SK1 composite pulse sequence that compensates amplitude errors to first order.

    SK1(theta) = R_0(theta), then R_{phi1}(2*pi), then R_{-phi1}(2*pi)

    where phi1 = arccos(-theta/(4*pi)).
    The three rotations are all about axes in the x-y plane but at different phases.

    The returned GatePulse has:
    - hamiltonian: the Hamiltonian for the first segment (R_0), used as the
      "representative" Hamiltonian. The actual composite sequence is stored
    ...
    gate = GatePulse(hamiltonian=H0, duration=total_duration)
    gate.pulses = pulses
    return gate
...
def bb1_composite_gate(
    ...
    """BB1 (Broadband-1) composite pulse: compensates amplitude errors to second order.
