...
def carrier_hamiltonian(
    ...
    """Carrier transition Hamiltonian: H = (Omega/2)(sigma+ e^{i*phi} + sigma- e^{-i*phi}).

    Drives |0> <-> |1> without changing the motional state.
    """
    sp = ops.sigma_plus(ion)
    sm = ops.sigma_minus(ion)
    return (rabi_frequency / 2) * (sp * np.exp(1j * phase) + sm * np.exp(-1j * phase))
...
def red_sideband_hamiltonian(
    ...
    """Red sideband Hamiltonian: H = (eta*Omega/2)(a * sigma_- + a_dag * sigma_+).

    Drives |0,n> <-> |1,n-1>: excites the qubit while removing one phonon.

    In QuTiP's convention sigma_- = |1><0| takes |0> -> |1> (excitation),
    and a removes a phonon, so the coupling term is a * sm + h.c.
    """
    sp = ops.sigma_plus(ion)
    sm = ops.sigma_minus(ion)
...
def blue_sideband_hamiltonian(
    ...
    """Blue sideband Hamiltonian: H = (eta*Omega/2)(a_dag * sigma_- + a * sigma_+).

    Drives |0,n> <-> |1,n+1>: excites the qubit while adding one phonon.

    In QuTiP's convention sigma_- = |1><0| takes |0> -> |1> (excitation),
    and a_dag adds a phonon, so the coupling term is ad * sm + h.c.
    """
    sp = ops.sigma_plus(ion)
    sm = ops.sigma_minus(ion)
    a = ops.annihilate(mode)
...
def full_interaction_hamiltonian(
    ...
    """Full laser-ion interaction in the interaction picture, returned as QuTiP list-format H.

    In the interaction picture with respect to the qubit and mode free evolution,
    the Hamiltonian becomes time-dependent. For first-order Lamb-Dicke:

    H(t) = (Omega/2) * sigma_x * cos(delta*t + phi)                    [carrier]
          + (eta*Omega/2) * (a*sigma+ + a_dag*sigma-) * e^{-i*omega_m*t} [RSB, rotating]
          + (eta*Omega/2) * (a_dag*sigma+ + a*sigma-) * e^{+i*omega_m*t} [BSB, rotating]

    For the lab-frame (interaction picture w.r.t. free Hamiltonian), we return:

    H = [H_drift, [H_rsb, coeff_rsb(t)], [H_bsb, coeff_bsb(t)], [H_carrier, coeff_carrier(t)]]
    ...
    lamb_dicke_order : int
        Order of Lamb-Dicke expansion (1 = standard, 2 = includes second-order corrections).
    ...
    # Red sideband: (eta*Omega/2) * (a * sm + h.c.) at detuning (delta - omega_m)
    ...
    # Blue sideband: (eta*Omega/2) * (ad * sm + h.c.) at detuning (delta + omega_m)
